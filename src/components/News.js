...
export default class News extends Component {
  ...
  handelPrev = async () =>{
    ...
    let parsedData = await data.json();
    this.setState(this.articles = parsedData.articles);
    this.setState(this.page = this.page - 1);
  }
  handleNext = async () =>{
    ...
    return (
      <div className='container my-3'>
        ...
        <div className="container d-flex justify-content-between">
          <button className="btn btn-dark btn-sm" onClick={this.handelPrev}>&larr; Previous</button>
          <Spinner />
          <button className="btn btn-dark btn-sm" onClick={this.handleNext}>Next &rarr;</button>
        </div>
      </div>
    )
  }
